/**
 * MistralReviewPanel - Panel de validation pour les documents générés par Mistral
 * Affiche les scores de confiance et les sources françaises
 */
...
import { MistralEnrichedDocument, MistralEnrichmentService } from '../../services/MistralEnrichmentService'

interface MistralReviewPanelProps {
  documents: MistralEnrichedDocument[]
  onBack: () => void
  onContinue: (approvedDocs: MistralEnrichedDocument[]) => void
  onDocumentsUpdated: (docs: MistralEnrichedDocument[]) => void
}

export function MistralReviewPanel({
  documents,
  onBack,
  onContinue,
  onDocumentsUpdated,
}: MistralReviewPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-4 border-b border-white/5">
        <h3 className="text-lg font-semibold text-white">Validation des fiches générées</h3>
        <p className="text-sm text-white/50 mt-1">
          Vérifiez les données générées par Mistral AI avant injection
        </p>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <div className="flex items-center gap-1.5">
                      <Shield className={cn('w-3 h-3', MistralEnrichmentService.getConfidenceColor(overallConfidence))} />
                      <span className={cn('text-xs', MistralEnrichmentService.getConfidenceColor(overallConfidence))}>
                        {overallConfidence}% - {MistralEnrichmentService.getConfidenceLabel(overallConfidence)}
                      </span>
                    </div>
                    {doc.sources.length > 0 && (
                      <div className="flex items-center gap-1 text-blue-400">
                        <BookOpen className="w-3 h-3" />
                        <span className="text-xs">{doc.sources.length} source{doc.sources.length > 1 ? 's' : ''}</span>
                      </div>
                    )}
                    {doc.warnings.length > 0 && (
                      <div className="flex items-center gap-1 text-amber-400">
                        <AlertTriangle className="w-3 h-3" />
                        <span className="text-xs">{doc.warnings.length} alerte{doc.warnings.length > 1 ? 's' : ''}</span>
                      </div>
                    )}
                  </div>
                </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {doc.sources.map((source, i) => (
                              ...
                              >
                                ...
                                <span className="text-xs text-blue-300">{source}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                          ...
                          {Object.entries(doc.confidence)
                            ...
                            .map(([key, value]) => (
                              ...
                              >
                                ...
                                <p className={cn('text-sm font-medium', MistralEnrichmentService.getConfidenceColor(value as number))}>
                                  {value}%
                                </p>
                              </div>
                            ))}
          ...
          <div className="flex items-center gap-4">
            {approvedCount > 0 && (
              <span className="text-sm text-white/50">
                {approvedCount} fiche{approvedCount > 1 ? 's' : ''} prête{approvedCount > 1 ? 's' : ''}
              </span>
            )}
            <button
